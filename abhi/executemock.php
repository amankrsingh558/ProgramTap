...
$folder1=$_SESSION['submissions'];
$question_id=$_SESSION['ques'];
//$question_id="mpst0001";
...
$file=$folder1.'/'.$question_id.'/'."code.c";
		//for insertion of submission_id and file location
		if(!file_exists("$folder1/$question_id")){
		mkdir("$folder1/$question_id",0777,true);
		 }
		...
		fwrite($fh1,$data);  
         fclose($fh1);		
$errorfile=$folder1.'/'.$question_id.'/'."error.txt";
$answers=$folder.$question_id.'/'."answers";
$testcases=$folder.$question_id.'/'."testcases";
$executable=$folder1.'/'.$question_id.'/a';
$output=$folder1.'/'.$question_id.'/output.txt';
$data2=0;
...
//passing the final value to the httpRequest
 ...
 if($correct==0){
		...
		$result=mysqli_query($con,"select count(*) as no ,s_id from submission where user_id='$userid' and course_id='$course_id' and exam_id='mock' and question_id='$question_id'");
while($row=mysqli_fetch_assoc($result))
    ...
    // header('location:hello.php');
       mysqli_query($con,"insert into submission (user_id,course_id,exam_id,question_id,submission_id,file_location,testcase_satisfied,testcase_not_satisfied) values ('$userid','$course_id','mock','$question_id','$submission_id','$file',$correct,$wrong1)");
		}
		mysqli_close($con);
