...
while($row=mysqli_fetch_assoc($result))
	{
		...
		if($nofsub==1){
		     ...
		     mysqli_query($con,"update submission set submission_id='$submission_id' ,submission_time=CURRENT_TIMESTAMP(),testcase_satisfied='$correct' ,testcase_not_satisfied='$wrong1' where s_id=$s_id");
			 while(mysqli_error($con)!==""){
				 $submission_id=rand(1000000,999999999);
				 mysqli_query($con,"update submission set submission_id='$submission_id' ,testcase_satisfied='$correct' ,testcase_not_satisfied='$wrong1' where s_id=$s_id");
			 }
		}
		else{
