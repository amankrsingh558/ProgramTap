	   ...
	   ?>
	     <td style="text-align: center;font-weight:normal;" onclick="" name="col" ><?php echo "$roww[2]"; ?></td>
		  <td  style="text-align: center;font-weight:normal;"onclick=")" name="col" ><?php echo "$roww[8]"; ?></td>
		    <td style="text-align: center;font-weight:normal;" onclick="" name="col" ><?php   $nofsub=0;
			if($type=="p"){
						$con1=mysqli_connect('localhost','root');
                         mysqli_select_db($con1,'compiler');
						$result1=mysqli_query($con1,"select count(*) as no,no_of_question from practical_exam where exam_id='$roww[1]'");
			...
			$result=mysqli_query($con,"select count(*) as no from mcq where eid='$roww[1]'");
			   ...
			   elseif($date>$today)
			  {
			     if($roww[9]==1)
			  {?>
			  <button type="button"   id="bt"  onclick="fetch(this,1)" value="Schedule Exam" class="btn btn-primary" style="background-color:green;width:140px;">Re-Schedule</button>
				
			  <?php }
			      else{
					  ?>
					  <button type="button"  onclick="fetch(this,0)" id="bt"  value="Schedule Exam" class="btn btn-primary" style="width:140px;">Shedule Exam</button>
				<?php  }
